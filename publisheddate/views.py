...
class book_manager(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if kwargs:
            return HttpResponse(
                json.dumps(books_list[kwargs['book_id']], indent=2),
        ...
        if filter_date_query:
            ...
            return HttpResponse(
                json.dumps(filtered_date_books_list, indent=2),
                content_type="application/json")
        if sort_query == 'published_date':
            return HttpResponse(
                json.dumps(sorted(books_list, key=self.get_date), indent=2),
                content_type="application/json")
        elif sort_query == '-published_date':
            return HttpResponse(
                json.dumps(
                    sorted(books_list, key=self.get_date, reverse=True), 
                    indent=2), 
                content_type="application/json")
        if filter_authors_query:
            ...
            return HttpResponse(
                json.dumps(filtered_authors_books_list, indent=2),
                content_type="application/json")
        return HttpResponse(
            json.dumps(books_list, indent=2), content_type="application/json")
